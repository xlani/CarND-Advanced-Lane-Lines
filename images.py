...
for idx, fname in enumerate(images):
    ...
    preprocess_img2 = np.zeros_like(img[:,:,0])
    preprocess_img3 = np.zeros_like(img[:,:,0])
    gradx = abs_sobel_thresh(img, orient = 'x', thresh = (30, 255))
    ...
    preprocess_img2[((gradx == 1) & (grady == 1))] = 255
    preprocess_img3[((s_binary == 1) & (v_binary == 1))] = 255
    result = preprocess_img
    result2 = preprocess_img2
    result3 = preprocess_img3
    ...
    result5 = cv2.addWeighted(warpage, 1, template, 0.5, 0.0) # overlay the orignal road image with window results
    ...
    result6 = cv2.addWeighted(img, 1.0, road_warped, 1.0, 0.0)
    ...
    cv2.putText(result6, 'Left Radius of Curvature = ' +str(round(l_curverad,3)) +'(m)', (25, 50), cv2.FONT_HERSHEY_PLAIN, 0.98, (255, 255, 255), 2)
    cv2.putText(result6, 'Right Radius of Curvature = ' +str(round(r_curverad,3)) +'(m)', (25, 75), cv2.FONT_HERSHEY_PLAIN, 0.98, (255, 255, 255), 2)
    cv2.putText(result6, 'Vehicle is ' +str(abs(round(center_diff,3))) +'m ' +side_pos +' of center', (25, 100), cv2.FONT_HERSHEY_PLAIN, 0.98, (255, 255, 255), 2)

    # plot undistorted image
    cv2.imwrite(fname[:-4]+'_grad_color.jpg', result)
    cv2.imwrite(fname[:-4]+'_gradient.jpg', result2)
    cv2.imwrite(fname[:-4]+'_colorspace.jpg', result3)
    cv2.imwrite(fname[:-4]+'_warped.jpg', result4)
    cv2.imwrite(fname[:-4]+'_lane_ident.jpg', result5)
    cv2.imwrite(fname[:-4]+'_final.jpg', result6)
    cv2.imwrite(fname[:-4]+'_undistorted.jpg', result7)
